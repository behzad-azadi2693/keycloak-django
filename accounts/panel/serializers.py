    ...
    def validate(self, attrs):
        ...
        if attrs["password"] != attrs["password_confierm"]:
            raise serializers.ValidationError(
                {"password_confierm": "password and password confierm is not matched"},
                code=400,
            )
        ...
        if user.get_user() != 404:
            raise serializers.ValidationError(
                {"username": "user with username is exsits"}, code=400
            )
        if phone or email:
            return attrs
        else:
            raise serializers.ValidationError(
                {"username": "username is not valid"}, code=400
            )
...
class UpdateUserSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if user.get_user() == 404:
            raise serializers.ValidationError(
                {"username": "user is not exsits"}, code=400
            )
        ...
        if "new_username" in attrs:
            ...
            if user.check_connect() == 500:
                raise serializers.ValidationError(
                    {"message": "server not found"}, code=500
                )
            if user.get_user() != 404:
                raise serializers.ValidationError(
                    {"username": "user with username is exsits"}, code=400
            ...
            if phone or email:
                ...
                raise serializers.ValidationError(
                    {"new_username": "username is not validate"}
                )
