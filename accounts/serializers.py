...
class SignupSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if user.check_email_verify() == 200:
            raise serializers.ValidationError(
                {"username": "User already exists"}, code=403
            )

        # Check if passwords match
        if attrs["password"] != attrs["password_confirm"]:
            raise serializers.ValidationError(
                {"password_confirm": "Passwords do not match"}, code=401
            )
        ...
        if phone or email:
            ...
            raise serializers.ValidationError(
                {"username": "Username is not correct"}, code=403
            )
...
class OTPRequestSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not phone and not email:
            raise serializers.ValidationError(
                {"message": "username is not correct"}, code=404
            )
        if user.check_connect() == 500:
            raise serializers.ValidationError({"message": "server not found"}, code=500)
        if user.get_user_id() == 404:
            raise serializers.ValidationError(
                {"message": "username not exists"}, code=401
            )
...
class OTPSingUpVerifySerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not phone and not email:
            raise serializers.ValidationError(
                {"message": "username is not correct"}, code=422
            )
...
class PasswordChangeSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if attrs["password"] != attrs["password_confirm"]:
            raise serializers.ValidationError(
                {"password confirm": "password not matched"}, code=400
            )
        return attrs
    ...
    def put(self, validated_data):
        ...
        user_keycloak.username = validated_data["username"]
